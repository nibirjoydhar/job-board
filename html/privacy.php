<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Privacy Policy</title>
    <?php include('headlink.php');?>

    <style>
    body {
        background-color: #f8f9fa;
    }



    .card {
        box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
        padding: 20px;
    }
    </style>
</head>

<body>
    <?php include('header.php'); ?>
    <div class="container">
        <div class="card">
            <h2 class="text-center">Privacy Policy</h2>
            <p>Your privacy is important to us. This policy explains how we handle your personal data.</p>
            <h4>Information Collection</h4>
            <p>We collect personal information when you use our services, such as your name, email, and contact details.
            </p>
            <h4>Data Usage</h4>
            <p>Your data is used to improve our services, process transactions, and communicate with you.</p>
            <h4>Data Protection</h4>
            <p>We implement security measures to protect your information from unauthorized access.</p>
            <h4>Contact Us</h4>
            <p>If you have any questions, please <a href="contact.php">contact us</a>.</p>
        </div>
    </div>

    <?php include('footer.php'); ?>


</body>

</html>